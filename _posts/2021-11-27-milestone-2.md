...
The Lasso method selects features based on penalizing and reducing the corresponding coefficients. The coefficients for insignificant features will eventually be reduced to zero and thus dropped. We used `sklearn.linear_model.Lasso` method with `alpha=0.001` to penalize the features coefficients of a trained linear model with L1 regularization method. The selected features by Lasso were (please note that feature names starting with `shot_` and `prev_event_` are dummy features for `secondary_type` and `prev_event_type` features respectively):
- `coordinate_x`
- `coordinate_y`
- `distance_from_net`
- `angle`
