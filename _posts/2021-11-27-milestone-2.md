...
- `coordinate_x`, `coordinate_y`: [`int`, `int`] two seperate columns for the x and y coordinates of the event respectively.
...
- `prev_event_time_diff`: [`int`] the time difference between current event and the preceding event in seconds.
...
## Q1: XGBoost Baseline
In this section we will train an XGBoost model with both the `distance_from_net` and `angle` features, which we will refer to as the `xgboost_baseline` model in the upcoming sections.
...
Here we inspect all the features that were generated in the advanced feature engineering part to see if there are any correlation between a pair of features. When a high correlation is found that means one of the two features is not important (i.e. redundant) and we choose one of the two to remove them.

In the following figure we see the correlation between all the features:
![Features Correlation](/Images/m2_q6_features_correlation.png)

As we can see, there is high correlation between `is_rebound` feature and two other features: (`rebound_angle` and `prev_event_SHOT`). Also the `period` feature is highly correlated with `game_sec`. And both `coordinate_x` and `coordinate_y` features are correlated with `distance_from_net` and `angle` features respectively. 

After removing the following features (`is_rebound`, `coordinate_x`, `coordinate_y`, `period`). Then tune an XGBoost model on the remaining features. The hyperparameter for the tuned model (using the same optimization process used in previous sections) are:
...
The trained model name is `xgboost_feats_non_corr` and in the following figures we see it surpases the best neural network model slightly on the AUC score:
![Best Perfomers](/Images/m2_q6_best_performers.png)
