...
### How?
Since all events are recorded by the time of happening, we can use occurrences of `Penalty` and other events to track the game strength and record it for each event. For the players strength (i.e. 5 on 4 or 5 on 5) a similar method is valid to track what is the players strength for each team at any event.
